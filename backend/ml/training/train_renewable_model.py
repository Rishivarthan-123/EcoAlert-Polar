...
def create_future_target(
    ...
    """
    Create a one-hour-ahead renewable generation target.
    """

    result = df.copy()

    result["target_renewable_next_hour"] = (
        result[TARGET].shift(-1)
    )

    result = result.dropna(
        subset=["target_renewable_next_hour"]
    ).reset_index(drop=True)
...
def main() -> None:
    ...
    print(
        "Target: next-hour renewable generation"
    )
    ...
    print()
    print(
        "Training XGBoost renewable model..."
    )
    ...
    artifact = {
        "model": model,
        "features": FEATURES,
        "target": "target_renewable_next_hour",
        "horizon_hours": 1,
    ...
    print()
    print(
        "Renewable energy model training completed."
    )
