...
class EcoLoadsPage {
      ...
      return `
        <tr>
          ...
          <td>
            ...
            >
              <button
                class="btn btn-secondary btn-sm btn-view-reason"
    ...
    if (action === "PROTECT") {
      return `
        <span
          ...
          style="
            color: var(--success-aurora);
            border-color: rgba(57, 230, 165, 0.4);
          "
        >
          🛡️ PROTECT
        </span>
  ...
  bindRowButtons() {
    const loads =
      window.ecoState.getState("loads") || [];

    document
      .querySelectorAll(".btn-view-reason")
      .forEach(btn => {

        btn.addEventListener("click", () => {

          const id =
            btn.getAttribute("data-load-id");
